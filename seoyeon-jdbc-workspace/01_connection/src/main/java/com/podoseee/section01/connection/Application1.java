...
public class Application1 {
    public static void main(String[] args) {
        ...
        try {
            ...
            conn = DriverManager.getConnection("jdbc:mysql://localhost/empdb", "podoseee", "podoseee");

            System.out.println("conn: " + conn);

        } catch (ClassNotFoundException | SQLException e) { // Driver 클래스를 찾지 못할 경우 발생되는 예외 (mysql-connector-j 라이브러리 등록안했거나, 오타있거나)
            e.printStackTrace();
        } finally{
            if(conn != null){
                try {
